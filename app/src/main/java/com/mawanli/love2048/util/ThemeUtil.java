...
public class ThemeUtil {
	...
	public static StateListDrawable whiteButtonBg(Context context, int radius) {
		...
		sld.addState(new int[] { android.R.attr.state_pressed },
				ShapeUtil.getWhiteStrokeButtonBg(getThemeColor(context), radius, context.getResources().getInteger(R.integer.big_stroke_width)));
		sld.addState(new int[] { 0 },
				ShapeUtil.getColorStrokeButtonBg(Color.WHITE, radius, context.getResources().getInteger(R.integer.big_stroke_width)));
		return sld;
	}
	...
	public static void addCardBackGround(ImageView imageView, int number, Context context) {
		...
		if (i < 16) {
			...
			if (getCardBackGroundType(context) == Constant.COLOR_CARD) {
				...
				imageView.setImageDrawable(ShapeUtil.getShape(context.getResources().getDimensionPixelSize(R.dimen.card_radius)-context.getResources().getDimensionPixelSize(R.dimen.card_stroke_with), getCardColor(i, context), 2, getCardColor(i, context)));
			...
			imageView.setImageDrawable(ShapeUtil.getShape(context.getResources().getDimensionPixelSize(R.dimen.card_radius)-context.getResources().getDimensionPixelSize(R.dimen.card_stroke_with), Color.BLACK, 0, 0));
	...
	public static StateListDrawable whiteTransparentButtonBg(Context context) {
		StateListDrawable sld = new StateListDrawable();
		sld.addState(new int[] { android.R.attr.state_pressed },
				ShapeUtil.getWhiteStrokeSubfillBg(R.dimen.big_radius, context.getResources().getInteger(R.integer.big_stroke_width)));
